...
import { applyItineraryOverrides, getItemReplaceability, getItemSlotId, restoreDay, restoreSlot, upsertOverride, type ItineraryOverride } from './utils/itineraryCustomizer'
import { checkInsertion } from './utils/itineraryConflictChecker'
...
function App() {
  ...
  const handleAddCandidate = (candidate: AlternativeAttraction, date: string) => {
    ...
    const dateIsKk = Boolean(dateToSabah(date))
    if (!day) { window.alert('没有找到这一天的行程。'); return }
    if (date === '9/13') { window.alert('9/13 早晨有09:20国际航班，不增加景点。'); return }
    if ((candidate.city === 'kota-kinabalu') !== dateIsKk) { window.alert('这个候选属于另一座城市，请选择与它同城的日期。'); return }
    if (candidate.timeScope === 'full-day' && dateIsKk) {
      const mainActivity = day.items.find(item => item.slotId?.startsWith('sabah-main-'))
      if (mainActivity) {
        window.alert(`当前${date}已有主活动「${mainActivity.title}」，没有舒服的全天空位。请点击这项 Timeline 的“调整”，用「${candidate.nameZh}」替换它，而不是继续增加景点。`)
        return
      }
    }
    const preferredStartTime = candidate.timeScope === 'full-day' || candidate.recommendedTime === 'morning' ? '07:00' : candidate.recommendedTime === 'evening' || candidate.recommendedTime === 'night' ? '17:00' : '14:00'
    const preferredStart = parseTime(preferredStartTime)
    const nextIndex = day.items.findIndex(item => parseTime(item.time) > preferredStart)
    const anchor = nextIndex > 0 ? day.items[nextIndex - 1] : nextIndex === 0 ? undefined : day.items[day.items.length - 1]
    const nextFixed = day.items.slice(Math.max(nextIndex, 0)).find(item => getItemReplaceability(item) === 'fixed')
    const anchorEnd = anchor ? parseTime(anchor.time) + durationMinutes(anchor) + (anchor.nextRouteMinutes ?? 0) : 0
    const startTime = formatTime(Math.max(preferredStart, anchorEnd))
    const routeMinutes = anchor ? estimateZoneTravel(candidate.city, zoneForItem(anchor), candidate.area).minutes : 8
    const insertion = checkInsertion({ date, startTime, durationMin: candidate.durationMin, slotType: candidate.timeScope, nextFixedTime: nextFixed?.time, routeMinutes })
    if (!insertion.canConfirm) { window.alert(insertion.blockers.join('\n')); return }
    const message = `建议插入 ${date} ${startTime}—${insertion.endTime}：${candidate.nameZh}\n${anchor ? '与“' + anchor.title + '”顺接' : '作为当天第一项'}，预计交通约${routeMinutes}分钟。\n${insertion.warnings.join('\n') || '不会碰到固定事项。'}\n\n确定写入自定义行程吗？`
    if (!window.confirm(message)) return
    setOverrides(previous => upsertOverride(previous, { date, slotId: `insert-${candidate.id}-${date}`, action: 'insert', attractionId: candidate.id, anchorSlotId: anchor ? getItemSlotId(anchor) : undefined, startTime }))
    goToDay(date)
  }
