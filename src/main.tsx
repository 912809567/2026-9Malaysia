...
import { AlternativeExplorer, defaultAlternativeFilters, type AlternativeFilterState } from './components/AlternativeExplorer'
import { ReplaceAttractionSheet } from './components/ReplaceAttractionSheet'
...
import { alternativeById, alternativeToDiscoverPlace, type AlternativeAttraction } from './data/alternatives'
import { applyItineraryOverrides, findBestInsertionGaps, getItemReplaceability, getItemSlotId, restoreDay, restoreSlot, upsertOverride, type ItineraryOverride } from './utils/itineraryCustomizer'
import { assessDayLoad } from './utils/itineraryConflictChecker'
import { durationMinutes, formatTime, parseTime, zoneForItem } from './utils/routeEstimator'
import { estimateTravel as estimateZoneTravel } from './data/travelZones'
import './styles.css'
...
const exportKeys = ['trip-itinerary-done', 'trip-sabah-weather', 'trip-sabah-plan', 'trip-sabah-plan-locked', 'trip-sabah-custom-locked', 'trip-custom-itinerary', 'trip-alternative-favorites', 'trip-alternative-filters', 'trip-sea-checks', 'trip-mangrove', 'trip-mangrove-day', 'trip-checks', 'trip-budget', 'trip-mdac', 'trip-entry-prep', 'trip-bookings', 'trip-favorites', 'trip-jetty', 'trip-travel-mode']
...
function buildDays(plan: SabahPlan, mangrove: boolean, mangroveDay: SabahDate, overrides: ItineraryOverride[] = []): DisplayDay[] {
  const baseDays: DayPlan[] = itinerary.map(day => {
    ...
    const customFullDay = typeof activity === 'object' && alternativeById[activity.attractionId]?.timeScope === 'full-day'
    if (date === '2026-09-12' && !customFullDay) {
      items.push({
        id: 'd12-sunset',
        date: day.date,
        time: '17:10',
  ...
  return applyItineraryOverrides(baseDays, overrides, alternativeById).map(day => ({ ...day, items: day.items.map(item => ({ ...item, date: day.date })) }))
}
...
function ActionDetail({ item, onMapFocus }: { item: DisplayItem; onMapFocus: (target: string) => void }) {
  ...
  return <div className="event-detail action-detail">
    ...
    {hasRoute && <div className="action-route">
      ...
      {item.arrivalTime && <div><span>{item.itemKind === 'activity' ? '抵达 / 开始游览' : '抵达'}</span><strong>{item.arrivalTime}</strong></div>}
...
type ItineraryProps = {
  ...
  onToday: () => void
  onOpenGallery: (placeId: string) => void
  onMapFocus: (target: string) => void
  overrides: ItineraryOverride[]
  onAdjust: (item: ItineraryItem, date: string, items: ItineraryItem[]) => void
  onRemove: (item: ItineraryItem, date: string) => void
  onRestoreSlot: (date: string, slotId: string) => void
  onRestoreDay: (date: string) => void
  onRestoreAll: () => void
  onMove: (date: string, slotId: string, targetSlotId: string) => void
}

function Itinerary({ days, selected, setSelected, done, setDone, onToday, onOpenGallery, onMapFocus, overrides, onAdjust, onRemove, onRestoreSlot, onRestoreDay, onRestoreAll, onMove }: ItineraryProps) {
  const [open, setOpen] = useState<string | null>('d7-1')
  const day = days[selected]
  const customDay = overrides.some(override => override.date === day.date) || day.items.some(item => item.source === 'custom')
  const load = assessDayLoad(day.items)
  const dateTabsRef = useRef<HTMLDivElement>(null)
  useEffect(() => {
    const tabs = dateTabsRef.current
    const activeTab = tabs?.querySelector<HTMLElement>('.date-tab.active')
    ...
    if (activeBounds.left < bounds.left || activeBounds.right > bounds.right) {
      tabs.scrollLeft += activeBounds.left - bounds.left - (bounds.width - activeBounds.width) / 2
    }
  }, [selected])
  return <section className="section" id="itinerary">
    <div className="itinerary-section-head"><SectionHead kicker="02 · ITINERARY / 行程" title="每天只看今天" note="每个节点都压缩成现场行动卡；调整只写入自定义层，默认推荐方案始终可以恢复。" /><button className="ghost-btn small" onClick={onRestoreAll}><RotateCcw size={12} />恢复全部默认</button></div>
    <div className="date-tabs" ref={dateTabsRef}><button className="today-btn" onClick={onToday}>📍 今天</button>{days.map((item, i) => { const isCustom = overrides.some(override => override.date === item.date) || item.items.some(entry => entry.source === 'custom'); return <button className={'date-tab ' + (selected === i ? 'active' : '') + (isCustom ? ' custom-tab' : '')} onClick={() => setSelected(i)} key={item.date}><strong>{item.date}</strong><span>{item.weekday}</span>{isCustom && <em>已自定义</em>}</button> })}</div>
    <div className="itinerary-layout"><div className="card"><div className="day-heading"><div><h3>{day.title}</h3>{customDay && <div className="custom-day-label">CUSTOM · 自定义层已生效</div>}</div><div className="day-heading-actions"><div className={'day-load-summary ' + (load.label === '舒服' ? 'comfortable' : load.label === '偏满' ? 'busy' : 'overloaded')}><strong>{load.label === '舒服' ? '🟢' : load.label === '偏满' ? '🟡' : '🔴'} {load.label}</strong><span>有效游玩＋交通约 {load.hours} 小时</span><small>连续户外约 {Math.round(load.maxOutdoorBlockMinutes / 10) / 10} 小时</small>{load.warnings[0] && <em>{load.warnings[0]}</em>}</div>{customDay && <button className="ghost-btn small" onClick={() => onRestoreDay(day.date)}><RotateCcw size={12} />恢复今天默认</button>}</div></div><div className="timeline">{day.items.map((item, index) => { const isOpen = open === item.id; const isDone = Boolean(done[item.id]); const replaceability = getItemReplaceability(item); const isCustom = item.source === 'custom'; const previous = day.items[index - 1]; const next = day.items[index + 1]; const slotId = getItemSlotId(item); const isSabahMain = slotId.startsWith('sabah-main-'); return <div className={'timeline-item ' + (item.tone || '') + (isCustom ? ' timeline-custom' : '')} key={item.id}><div className="timeline-main"><div className="time">{item.time}</div><div className={'event ' + (isOpen ? 'open' : '')}><button type="button" onClick={() => setOpen(isOpen ? null : item.id)} aria-expanded={isOpen}><div className="event-title"><span>{item.title}</span><ChevronDown size={16} /></div><div className="summary">{item.summary}</div></button><div className="timeline-item-meta">{replaceability === 'fixed' ? <span className="fixed-badge">🔒 固定事项</span> : <button className="adjust-btn" type="button" onClick={() => onAdjust(item, day.date, day.items)}>调整</button>}{isCustom && <button className="restore-item-btn" type="button" onClick={() => onRestoreSlot(day.date, slotId)}>{isSabahMain ? '恢复默认' : '恢复此项'}</button>}{replaceability !== 'fixed' && !isSabahMain && <><button className="move-btn" type="button" disabled={!previous || getItemReplaceability(previous) === 'fixed'} onClick={() => previous && onMove(day.date, slotId, getItemSlotId(previous))} aria-label="前移">↑</button><button className="move-btn" type="button" disabled={!next || getItemReplaceability(next) === 'fixed'} onClick={() => next && onMove(day.date, slotId, getItemSlotId(next))} aria-label="后移">↓</button><button className="remove-btn" type="button" onClick={() => onRemove(item, day.date)}>删除</button></>}</div>{isOpen && <><ActionDetail item={item} onMapFocus={onMapFocus} />{item.image && <div className="timeline-media">{item.galleryPlaceId ? <button className="timeline-image-button" onClick={event => { event.stopPropagation(); onOpenGallery(item.galleryPlaceId!) }} aria-label={'打开' + item.title + '相册'}><ImageWithFallback className="timeline-thumb" src={item.image} alt={item.title} /></button> : <ImageWithFallback className="timeline-thumb" src={item.image} alt={item.title} />}</div>}</>}</div><button type="button" className={'complete-btn ' + (isDone ? 'done' : '')} onClick={() => setDone(prev => ({ ...prev, [item.id]: !isDone }))}>{isDone ? <Check size={14} /> : '完成'}</button></div></div> })}</div></div><FlightMini /></div>
  </section>
}
...
function Planner({ weather, setWeather, plan, setPlan, locked, setLocked, customLocked, setCustomLocked }: { weather: SabahWeather; setWeather: React.Dispatch<React.SetStateAction<SabahWeather>>; plan: SabahPlan; setPlan: React.Dispatch<React.SetStateAction<SabahPlan>>; locked: boolean; setLocked: React.Dispatch<React.SetStateAction<boolean>>; customLocked: boolean; setCustomLocked: React.Dispatch<React.SetStateAction<boolean>> }) {
  const preview = useMemo(() => planSabah(weather), [weather])
  const allBad = sabahDates.every(day => weather[day] === 'bad')
  const hasCustomPlan = customLocked || sabahDates.some(day => typeof plan[day] === 'object')
  const planLocked = locked || hasCustomPlan
  const updateWeather = (day: SabahDate, value: 'good' | 'okay' | 'bad') => { const next = { ...weather, [day]: value }; setWeather(next); if (!planLocked) setPlan(planSabah(next)) }
  return <section className="section" id="planner"><SectionHead kicker="04 · WEATHER / 海况排期" title="亚庇出海排期助手" note="最早的“海况优秀”优先给环滩岛；用户自定义全天项目会锁定在当前计划，不会被天气助手偷偷改回。" /><div className="planner"><div className="card"><div className="weather-grid">{sabahDates.map(day => <div className="weather-day" key={day}><strong>{sabahShort[day]}</strong><select className="weather-select" value={weather[day]} onChange={event => updateWeather(day, event.target.value as 'good' | 'okay' | 'bad')} disabled={planLocked}>{Object.entries(weatherLabel).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></div>)}</div><div className="weather-option"><span className="micro">{hasCustomPlan ? '当前计划含自定义项目，天气只作参考' : locked ? '当前方案已锁定，天气选择仅作预览' : '选择天气后会实时更新当前方案'}</span><div className="planner-actions"><button className="ghost-btn small" onClick={() => { setWeather(defaultSabahWeather); setPlan(planSabah(defaultSabahWeather)); setLocked(false); setCustomLocked(false) }}><RotateCcw size={13} />恢复默认</button>{planLocked ? <button className="ghost-btn small" onClick={() => { setPlan(preview); setLocked(false); setCustomLocked(false) }}>交回天气助手规划</button> : <button className="primary-btn small" onClick={() => { setPlan(preview); setLocked(true) }}>锁定此方案</button>}</div></div></div><div className="recommend"><div className="section-kicker">{planLocked ? '方案已锁定 · LOCKED' : '实时推荐 · LIVE'}</div><h3>{planLocked ? '当前执行方案' : '根据当前选择，推荐：'}</h3>{sabahDates.map(day => <div className="recommend-row" key={day}><strong>{sabahShort[day]}</strong><span>{day === '2026-09-11' && <b>换酒店＋</b>}{activityLabel(planLocked ? plan[day] : preview[day])}{hasCustomPlan && typeof plan[day] === 'object' && <small> · 已自定义</small>}</span></div>)}{allBad && <div className="no-go">海况不佳，不强行远海。建议市区、酒店、按摩、海鲜、榴莲，根据运营商通知决定是否参加TARP。</div>}</div></div></section>
}
...
function SeaModules({ plan, weather, setPlan, customLocked, mangrove, setMangrove, mangroveDay, setMangroveDay }: { plan: SabahPlan; weather: SabahWeather; setPlan: React.Dispatch<React.SetStateAction<SabahPlan>>; customLocked: boolean; mangrove: boolean; setMangrove: React.Dispatch<React.SetStateAction<boolean>>; mangroveDay: SabahDate; setMangroveDay: React.Dispatch<React.SetStateAction<SabahDate>> }) {
  ...
  const toggleMangrove = (checked: boolean) => { setMangrove(checked); if (checked) { if (plan[mangroveDay] === 'rest' || plan[mangroveDay] === 'city') setPlan(prev => ({ ...prev, [mangroveDay]: 'mangrove' })) } else if (!customLocked) setPlan(planSabah(weather)) }
...
function QuickModules({ jetty, setJetty, focusTarget, mapItems }: { jetty: { name: string; url: string }; setJetty: React.Dispatch<React.SetStateAction<{ name: string; url: string }>>; focusTarget?: string; mapItems: ItineraryItem[] }) {
...
function App() {
  ...
  const [customLocked, setCustomLocked] = useLocalStorage('trip-sabah-custom-locked', false)
  const [overrides, setOverrides] = useLocalStorage<ItineraryOverride[]>('trip-custom-itinerary', [])
  const [alternativeFavorites, setAlternativeFavorites] = useLocalStorage<string[]>('trip-alternative-favorites', [])
  const [alternativeFilters, setAlternativeFilters] = useLocalStorage<AlternativeFilterState>('trip-alternative-filters', defaultAlternativeFilters)
  ...
  const [replaceTarget, setReplaceTarget] = useState<{ item: ItineraryItem; date: string; items: ItineraryItem[] } | null>(null)
  const days = useMemo(() => buildDays(plan, mangrove, mangroveDay, overrides), [plan, mangrove, mangroveDay, overrides])
  ...
  const nav = [{ id: 'overview', label: '首页', icon: Compass }, { id: 'itinerary', label: '行程', icon: CalendarDays }, { id: 'alternatives', label: '自选', icon: Heart }, { id: 'checklist', label: '清单', icon: ListChecks }, { id: 'budget', label: '预算', icon: WalletCards }, { id: 'quick', label: '更多', icon: Landmark }]
  useEffect(() => {
    const observer = new IntersectionObserver(entries => {
      const visible = entries.filter(entry => entry.isIntersecting).sort((a, b) => b.intersectionRatio - a.intersectionRatio)[0]
      if (visible) setActive(visible.target.id)
    }, { rootMargin: '-15% 0px -65% 0px', threshold: [0, .25, .5] })
    ;['entry-prep', 'overview', 'itinerary', 'alternatives', 'checklist', 'budget', 'quick', 'safety'].forEach(id => { const element = document.getElementById(id); if (element) observer.observe(element) })
  ...
  const openGallery = (placeOrId: string | DiscoverPlace, index = 0) => {
    const place = typeof placeOrId === 'string'
      ? placeOrId.startsWith('alternative:')
        ? alternativeById[placeOrId.slice('alternative:'.length)] ? alternativeToDiscoverPlace(alternativeById[placeOrId.slice('alternative:'.length)]) : undefined
        : discoverPlaces.find(item => item.id === placeOrId)
      : placeOrId
    if (place) setGallery({ place, index })
  }
  const onToday = () => goToDay(today)
  const onMapFocus = (target: string) => { setMapFocus(target); go('quick') }
  const dateToSabah = (date: string): SabahDate | undefined => sabahDates.find(item => sabahShort[item] === date)
  const handleAdjust = (item: ItineraryItem, date: string, items: ItineraryItem[]) => setReplaceTarget({ item, date, items })
  const handleConfirmOverride = (override: ItineraryOverride) => {
    const candidate = override.attractionId ? alternativeById[override.attractionId] : undefined
    const sabahDate = dateToSabah(override.date)
    if (candidate && sabahDate && override.slotId.startsWith('sabah-main-') && candidate.city === 'kota-kinabalu') {
      setPlan(previous => ({ ...previous, [sabahDate]: { source: 'alternative', attractionId: candidate.id } }))
      setCustomLocked(true)
      setLocked(true)
      setReplaceTarget(null)
      goToDay(override.date)
      return
    }
    setOverrides(previous => upsertOverride(previous, override))
    setReplaceTarget(null)
    goToDay(override.date)
  }
  const handleRemove = (item: ItineraryItem, date: string) => {
    if (getItemReplaceability(item) === 'fixed') { window.alert('这是固定事项，不能删除。'); return }
    const slotId = getItemSlotId(item)
    if (slotId.startsWith('sabah-main-')) { window.alert('这是亚庇主活动槽位，请用“调整”更换，或恢复默认，不直接删除主活动。'); return }
    setOverrides(previous => upsertOverride(previous, { date, slotId, action: 'remove', removedTitle: item.title }))
  }
  const handleRestoreSlot = (date: string, slotId: string) => {
    const sabahDate = dateToSabah(date)
    if (sabahDate && slotId.startsWith('sabah-main-')) {
      setPlan(previous => ({ ...previous, [sabahDate]: defaultSabahPlan[sabahDate] }))
      setCustomLocked(false)
      setLocked(false)
      return
    }
    setOverrides(previous => restoreSlot(previous, date, slotId))
  }
  const handleRestoreDay = (date: string) => {
    setOverrides(previous => restoreDay(previous, date))
    const sabahDate = dateToSabah(date)
    if (sabahDate) {
      setPlan(previous => ({ ...previous, [sabahDate]: defaultSabahPlan[sabahDate] }))
      setCustomLocked(false)
  ...
  const handleRestoreAll = () => {
    setOverrides([])
    setPlan(defaultSabahPlan)
    setCustomLocked(false)
    setLocked(false)
  }
  const handleMove = (date: string, slotId: string, targetSlotId: string) => setOverrides(previous => upsertOverride(previous, { date, slotId, targetSlotId, action: 'move' }))
  const handleAddCandidate = (candidate: AlternativeAttraction, date: string) => {
    const day = days.find(item => item.date === date)
    const sabahDate = dateToSabah(date)
    if (!day) { window.alert('没有找到这一天的行程。'); return }
    if (date === '9/13') { window.alert('9/13 早晨有09:20国际航班，不增加景点。'); return }
    if (date === '9/9' && candidate.city === 'kota-kinabalu') {
      window.alert('虽然已抵达亚庇，但9/9还要经过沙巴入境、取行李、Grab和酒店入住；不建议再增加正式景点。请从9/10开始安排亚庇候选。')
      return
    }
    const expectedCity = sabahDate || date === '9/9' ? 'kota-kinabalu' : 'kuala-lumpur'
    if (date !== '9/9' && candidate.city !== expectedCity) {
      window.alert('这个候选属于另一座城市，请选择与它同城的日期。')
      return
    }
    const gaps = findBestInsertionGaps(day.items, candidate, date, sabahDate ? weather[sabahDate] : undefined)
    if (!gaps.length) {
      const mainActivity = day.items.find(item => item.slotId?.startsWith('sabah-main-'))
      if (candidate.timeScope === 'full-day' && mainActivity) {
        window.alert('当前' + date + '已有主活动「' + mainActivity.title + '」，当天没有舒服的全天空位。请点击 Timeline 的“调整”，用「' + candidate.nameZh + '」替换主活动，而不是继续增加景点。')
      } else if (date === '9/9' && candidate.city === 'kuala-lumpur') {
        window.alert('9/9 航班前还要保留机场办理缓冲；当天没有舒服的新增空位，建议替换黑风洞或保留移动日节奏。')
      } else {
        window.alert('当天没有舒服的新增空位，建议替换现有行程，而不是继续塞景点。')
      }
      return
    }
    const best = gaps[0]
    const alternatives = gaps.slice(1, 3).map(gap => gap.startTime + '—' + gap.endTime + '（' + (gap.routeQuality === 'green' ? '很顺路' : '可以安排') + '）').join('；')
    const message = [
      '推荐插入 ' + date + ' ' + best.startTime + '—' + best.endTime + '：' + candidate.nameZh,
      '前往候选约' + best.routeToCandidate.minutes + '分钟' + (best.routeToNext ? '，前往下一项约' + best.routeToNext.minutes + '分钟' : '') + '；' + (best.routeQuality === 'green' ? '🟢 很顺路' : '🟡 可以安排'),
      '推荐理由：' + best.reasons.join('；'),
      alternatives ? '备选时间：' + alternatives : '',
      best.report.warnings.length ? best.report.warnings.join('\n') : '不会碰到已知固定事项。',
      '',
      '确定写入自定义行程吗？',
    ].filter(Boolean).join('\n')
    if (!window.confirm(message)) return
    setOverrides(previous => upsertOverride(previous, {
      date,
      slotId: 'insert-' + candidate.id + '-' + date,
      action: 'insert',
      attractionId: candidate.id,
      anchorSlotId: best.anchorSlotId,
      beforeSlotId: best.beforeSlotId,
  ...
  return <div className={'app ' + (travelMode ? 'travel-mode' : '')}><div className="container"><div className="topbar"><div className="brand"><div className="brand-mark">✦</div><div>双人旅行攻略 <span>TRAVEL DOSSIER</span></div></div><div className="top-actions"><ScrollButton target="entry-prep">入境准备</ScrollButton><ScrollButton target="alternatives">自选景点</ScrollButton><button className="icon-btn" title="查看提醒" onClick={() => go('safety')}><ShieldCheck size={17} /></button><button className="ghost-btn small" onClick={resetTripData}><RotateCcw size={13} />重置数据</button></div></div><Hero days={days} travelMode={travelMode} setTravelMode={setTravelMode} /><EntryPrep /><Overview /><Itinerary days={days} selected={selected} setSelected={setSelected} done={done} setDone={setDone} onToday={onToday} onOpenGallery={openGallery} onMapFocus={onMapFocus} overrides={overrides} onAdjust={handleAdjust} onRemove={handleRemove} onRestoreSlot={handleRestoreSlot} onRestoreDay={handleRestoreDay} onRestoreAll={handleRestoreAll} onMove={handleMove} /><Hotels /><Planner weather={weather} setWeather={setWeather} plan={plan} setPlan={setPlan} locked={locked} setLocked={setLocked} customLocked={customLocked} setCustomLocked={setCustomLocked} /><BookingCenter /><SeaModules plan={plan} weather={weather} setPlan={setPlan} customLocked={customLocked} mangrove={mangrove} setMangrove={setMangrove} mangroveDay={mangroveDay} setMangroveDay={setMangroveDay} /><Checklist /><Budget /><QuickModules jetty={jetty} setJetty={setJetty} focusTarget={mapFocus} mapItems={days.flatMap(day => day.items)} /><AlternativeExplorer filters={alternativeFilters} setFilters={setAlternativeFilters} favorites={alternativeFavorites} setFavorites={setAlternativeFavorites} onOpenGallery={(place, index = 0) => setGallery({ place, index })} onRequestAdd={handleAddCandidate} /><Discover plan={plan} mangrove={mangrove} mangroveDay={mangroveDay} onOpenDay={goToDay} onOpenGallery={(place, index = 0) => setGallery({ place, index })} /><Safety /><footer className="footer">马来西亚 2026 · 深圳—吉隆坡—亚庇<br />图片均已本地化并保留可追溯来源；天气、海况、航班请在出发前再次确认。<br /><button className="text-link" onClick={() => { const event = new CustomEvent('open-export'); window.dispatchEvent(event) }}><FileJson size={12} />导出同行状态</button></footer></div>{gallery && <GalleryLightbox place={gallery.place} initialIndex={gallery.index} onClose={() => setGallery(null)} />}{replaceTarget && <ReplaceAttractionSheet date={replaceTarget.date} current={replaceTarget.item} items={replaceTarget.items} weather={dateToSabah(replaceTarget.date) ? weather[dateToSabah(replaceTarget.date)!] : undefined} onClose={() => setReplaceTarget(null)} onOpenGallery={(place, index = 0) => setGallery({ place, index })} onConfirm={handleConfirmOverride} />}<nav className="bottom-nav" aria-label="主导航">{nav.map(({ id, label, icon: Icon }) => <button className={active === id ? 'active' : ''} key={id} onClick={() => go(id)}><Icon size={17} />{label}</button>)}</nav></div>
}
