...
const weatherLabel: Record<'good' | 'okay' | 'bad', string> = { good: '☀️ 海况优秀', okay: '🌤️ 一般', bad: '🌧️ 不适合远海' }
  ...
  return itinerary.map(day => {
    ...
    if (date === '2026-09-11') {
      items.push({
        ...
        date: day.date,
        time: '09:00',
        title: 'Sheraton → Hyatt换酒店 · 行李先行',
        summary: '约09:00退房 → 全部行李Grab → Hyatt寄存 → 再去当天主活动。',
        details: ['不要默认06:30退房；以约09:00为舒适起点', '人和全部行李先到Hyatt，前台寄存后再去Klias / 市区备选', '晚上回Hyatt正式入住'],
        ...
        onSiteSteps: ['确认Sheraton房间和行李数量', '叫Grab并核对车牌', 'Hyatt前台寄存全部行李并拍照留凭'],
        whatToBring: ['护照', '手机充电宝', '当天活动订单'],
        fallbackPlan: '如入住手续或交通延误，改为Hyatt周边咖啡和市区休闲。',
    ...
    if (date === '2026-09-12') {
      items.push({
        ...
        details: ['远海日请根据返程时间决定是否赶，也可调整到其他空闲傍晚', '日落后：海鲜 → 沙巴特色榴莲 → Hyatt → 收拾行李'],
        ...
        fallbackPlan: '天气不佳时改为KK Waterfront或酒店附近晚餐。',
...
function ActionDetail({ item, onMapFocus }: { item: DisplayItem; onMapFocus: (target: string) => void }) {
  ...
  return <div className="event-detail action-detail">
    <div className="daily-brief"><strong>Daily Brief</strong><span>{item.summary}</span></div>
    <ActionBadge status={item.reservationStatus} />
    {hasRoute && <div className="action-route">
      {item.from && <div><span>FROM</span><strong>{item.from}</strong></div>}
      {item.to && <div><span>TO</span><strong>{item.to}</strong></div>}
    ...
    {(item.dressCode || item.paymentTip || item.weatherRisk || item.lastReturnTime || item.fallbackPlan || item.notes) && <div className="action-section">
      ...
      {item.fallbackPlan && <p>Plan B：{item.fallbackPlan}</p>}
...
function NextEvent({ days }: { days: DisplayDay[] }) {
  ...
  if (status.kind === 'after') return <div className="next-card complete-next"><div className="section-kicker">NEXT · 下一项</div><strong>旅程已完成</strong><span>把照片和回忆收好，下次再出发。</span></div>
  const now = new Date()
  ...
  if (!event) return <div className="next-card complete-next"><div className="section-kicker">NEXT · 下一项</div><strong>今日行程已完成，早点休息。</strong></div>
  const mins = Math.max(0, Math.round((new Date(event.dateTime).getTime() - now.getTime()) / 60000))
  return <div className="next-card"><div><div className="section-kicker">NEXT · 下一项</div><strong>{event.time} · {event.title}</strong><span>{status.kind === 'before' ? '下一项：' + event.date + ' ' + event.time : mins < 60 ? '还有' + mins + '分钟' : '还有' + Math.floor(mins / 60) + '小时' + mins % 60 + '分钟'}</span></div>{event.tone === 'warning' && <AlertTriangle color="var(--coral)" />}</div>
}

function Hero({ days, travelMode, setTravelMode }: { days: DisplayDay[]; travelMode: boolean; setTravelMode: (value: boolean) => void }) {
  const status = tripStatus()
  return <><header className="hero" style={{ backgroundImage: 'linear-gradient(115deg,rgba(6,69,72,.96) 5%,rgba(13,104,109,.83) 48%,rgba(13,104,109,.28)), url(' + resolveImageSrc('images/places/petronas/night-01.webp') + ')' }}><div className="hero-content"><div className="eyebrow"><Compass size={15} /> 2026 · FIELD GUIDE · MALAYSIA</div><h1>{trip.title}<br /><em>{trip.subtitle}</em></h1><p>深圳—吉隆坡—亚庇 · 09.07—09.13 · {trip.days}天{trip.nights}晚</p><div className="hero-bottom"><div className="status-pill"><CalendarDays size={15} />{status.label}</div><div className="route-line">{trip.route.map((stop, i) => <span key={stop.code + '-' + i} style={{ display: 'contents' }}><span className="route-dot" /><span>{stop.code}</span>{i < trip.route.length - 1 && <span className="route-sep" />}</span>)}</div></div><label className="travel-toggle"><input type="checkbox" checked={travelMode} onChange={event => setTravelMode(event.target.checked)} /><span>旅行模式</span><small>{travelMode ? '已开启 · 只保留现场信息' : '开启后简化页面'}</small></label></div></header><NextEvent days={days} /></>
}
...
function EntryPrep() {
  ...
  return <section className="section" id="entry-prep"><SectionHead kicker="00 · departure prep" title="中国大陆游客 · 入境准备" note="把入境材料离线放好，9/7落地时只按动线执行。政策和开放时间以官方最新页面为准。" /><div className="entry-prep-grid"><div className="card entry-prep-card"><div className="entry-progress"><strong>{completed}/{total}</strong><span>出发前准备完成</span></div><div className="entry-list">{entryPrepItems.map(item => <label className="list-check" key={item.id}><input type="checkbox" checked={Boolean(checks[item.id])} onChange={() => setChecks(prev => ({ ...prev, [item.id]: !prev[item.id] }))} /><span><strong>{item.title}</strong><small>{item.copy}</small>{item.link && <a href={item.link} target="_blank" rel="noreferrer" onClick={event => event.stopPropagation()}>{item.linkLabel} <ExternalLink size={11} /></a>}</span></label>)}<label className="list-check"><input type="checkbox" checked={mdac} onChange={event => setMdac(event.target.checked)} /><span><strong>MDAC已提交</strong><small>9/5起可填，推荐9/5或9/6完成；提交后截图、PDF、纸质各备一份。</small><a href="https://imigresen-online.imi.gov.my/mdac/main" target="_blank" rel="noreferrer" onClick={event => event.stopPropagation()}>前往官方 MDAC 入口 <ExternalLink size={11} /></a></span></label></div></div><div className="card entry-note-card"><ShieldCheck size={22} color="var(--teal)" /><h3>9/9 沙巴入境提示</h3><p>从KUL飞抵BKI后，先找 Immigration / 沙巴移民检查，再取行李、确认护照记录，最后叫Grab去Sheraton。</p><div className="entry-mini"><span>护照</span><span>三段机票</span><span>三家酒店</span><span>保险</span></div><div className="micro">以马来西亚移民局最新政策为准</div></div></div></section>
}

function Overview() {
  return <section className="section" id="overview"><SectionHead kicker="01 · overview" title="先看一眼全程" note="城市与海岛的节奏，留出午休和天气弹性，不做特种兵。" /><div className="overview-grid"><div className="card route-card">{trip.route.map((stop, i) => <span className="route-stop" key={stop.code + i}><strong>{stop.code}</strong><span>{stop.name}</span>{(i === 1 || i === 2) && <small style={{ display: 'block', color: 'var(--teal)', marginTop: 8 }}>{i === 1 ? '2晚' : '4晚'}</small>}</span>)}</div><div className="card trip-structure-card"><div className="metric-label">7 DAYS · 6 NIGHTS</div><div className="structure-stops"><div><strong>KUALA LUMPUR</strong><span>2 NIGHTS</span></div><div className="structure-divider" aria-hidden="true">→</div><div><strong>KOTA KINABALU</strong><span>4 NIGHTS</span></div></div><div className="tag-list">{trip.tags.map(tag => <span className="tag" key={tag}>{tag}</span>)}</div></div></div></section>
}
...
function Itinerary({ days, selected, setSelected, done, setDone, onToday, onOpenGallery, onMapFocus }: { days: DisplayDay[]; selected: number; setSelected: (value: number) => void; done: Record<string, boolean>; setDone: React.Dispatch<React.SetStateAction<Record<string, boolean>>>; onToday: () => void; onOpenGallery: (placeId: string) => void; onMapFocus: (target: string) => void }) {
  ...
  return <section className="section" id="itinerary"><SectionHead kicker="02 · itinerary" title="每天只看今天" note="每个节点都压缩成现场行动卡：怎么走、要不要预约、到场做什么、下雨怎么办。" /><div className="date-tabs"><button className="today-btn" onClick={onToday}>📍 今天</button>{days.map((item, i) => <button className={'date-tab ' + (selected === i ? 'active' : '')} onClick={() => setSelected(i)} key={item.date}><strong>{item.date}</strong><span>{item.weekday}</span></button>)}</div><div className="itinerary-layout"><div className="card"><div className="day-heading"><h3>{day.title}</h3><span className="intensity">{day.intensity}</span></div><div className="timeline">{day.items.map(item => { const isOpen = open === item.id; const isDone = Boolean(done[item.id]); return <div className={'timeline-item ' + (item.tone || '')} key={item.id}><div className="timeline-main"><div className="time">{item.time}</div><div className={'event ' + (isOpen ? 'open' : '')}><button type="button" onClick={() => setOpen(isOpen ? null : item.id)} aria-expanded={isOpen}><div className="event-title"><span>{item.title}</span><ChevronDown size={16} /></div><div className="summary">{item.summary}</div></button>{isOpen && <><div className="event-detail action-detail"><div className="daily-brief"><strong>Daily Brief</strong><span>{item.summary}</span></div><ActionBadge status={item.reservationStatus} />{(item.from || item.to || item.transportMode || item.distance || item.duration) && <div className="action-route">{item.from && <div><span>FROM</span><strong>{item.from}</strong></div>}{item.to && <div><span>TO</span><strong>{item.to}</strong></div>}{item.transportMode && <div><span>交通方式</span><strong>{item.transportMode}</strong></div>}{item.distance && <div><span>距离</span><strong>{item.distance}</strong></div>}{item.duration && <div><span>时长</span><strong>{item.duration}</strong></div>}{item.recommendedDepartureTime && <div><span>建议出发</span><strong>{item.recommendedDepartureTime}</strong></div>}{item.arrivalTime && <div><span>抵达</span><strong>{item.arrivalTime}</strong></div>}{item.buffer && <div><span>缓冲</span><strong>{item.buffer}</strong></div>}</div>}<div className="action-copy">{item.details.map(detail => <div key={detail}>· {detail}</div>)}</div>{(item.reservationTiming || item.bookingChannel || item.meetingPoint || item.ticketOrFee) && <div className="action-section"><strong>预约 / 费用</strong>{item.reservationTiming && <p>{item.reservationTiming}</p>}{item.bookingChannel && <p>渠道：{item.bookingChannel}</p>}{item.meetingPoint && <p>集合：{item.meetingPoint}</p>}{item.ticketOrFee && <p>费用：{item.ticketOrFee}</p>}{item.bookingLink && <a className="action-link" href={item.bookingLink} target="_blank" rel="noreferrer">打开预订或官方页面 <ExternalLink size={12} /></a>}</div>}<ActionList title="到现场 1 · 2 · 3" items={item.onSiteSteps} /><ActionList title="携带物品" items={item.whatToBring} />{item.documents?.length ? <ActionList title="随身文件" items={item.documents} /> : null}{(item.dressCode || item.paymentTip || item.weatherRisk || item.lastReturnTime || item.fallbackPlan || item.notes) && <div className="action-section"><strong>现场提醒</strong>{item.dressCode && <p>着装：{item.dressCode}</p>}{item.paymentTip && <p>支付：{item.paymentTip}</p>}{item.weatherRisk && <p>天气：{item.weatherRisk}</p>}{item.lastReturnTime && <p>最晚返程：{item.lastReturnTime}</p>}{item.fallbackPlan && <p>Plan B：{item.fallbackPlan}</p>}{item.notes && <p>备注：{item.notes}</p>}</div>}{item.mapTarget && <button className="action-map-button" onClick={event => { event.stopPropagation(); onMapFocus(item.mapTarget!) }}><Map size={13} />查看地图 · {item.mapTarget}</button>}{(item.sourceName || item.verifiedAt) && <div className="action-source"><span>来源：{item.sourceName || '行程核验'}{item.verifiedAt ? ' · ' + item.verifiedAt : ''}</span>{item.sourceLink && <a href={item.sourceLink} target="_blank" rel="noreferrer">官方来源 ↗</a>}</div>}</div>{item.image && <div className="timeline-media">{item.galleryPlaceId ? <button className="timeline-image-button" onClick={event => { event.stopPropagation(); onOpenGallery(item.galleryPlaceId!) }} aria-label={'打开' + item.title + '相册'}><ImageWithFallback className="timeline-thumb" src={item.image} alt={item.title} /></button> : <ImageWithFallback className="timeline-thumb" src={item.image} alt={item.title} />}</div>}</>}</div><button type="button" className={'complete-btn ' + (isDone ? 'done' : '')} onClick={() => setDone(prev => ({ ...prev, [item.id]: !isDone }))}>{isDone ? <Check size={14} /> : '完成'}</button></div></div> })}</div></div><FlightMini /></div></section>
}
...
function BookingCenter() {
  ...
  return <section className="section" id="bookings"><SectionHead kicker="05 · book ahead" title="订票中心" note="先处理有明确时间窗的项目；打开链接后把订单号和确认消息离线保存。" /><div className="booking-grid">{bookingItems.map(item => <div className={'card booking-card ' + (done[item.id] ? 'booking-done' : '')} key={item.id}><div className="booking-top"><span className={'booking-status ' + item.status}>{item.status === 'must' ? '必须' : item.status === 'recommended' ? '建议' : '核对'}</span><label className="toggle"><input type="checkbox" checked={Boolean(done[item.id])} onChange={() => setDone(prev => ({ ...prev, [item.id]: !prev[item.id] }))} />{done[item.id] ? '已完成' : '标记完成'}</label></div><h3>{item.title}</h3><div className="micro">{item.timing}</div><p>{item.note}</p><div className="booking-channel">渠道：{item.channel}</div>{item.fee && <div className="booking-fee">费用：{item.fee}</div>}<a className="primary-btn small" href={item.link} target="_blank" rel="noreferrer">打开链接 <ExternalLink size={13} /></a></div>)}</div><div className="booking-summary"><Check size={15} /> 已完成 {completed}/{bookingItems.length} 项 · 订单确认后仍需按前一晚消息核对接送和海况</div></section>
}
...
function QuickModules({ jetty, setJetty, focusTarget }: { jetty: { name: string; url: string }; setJetty: React.Dispatch<React.SetStateAction<{ name: string; url: string }>>; focusTarget?: string }) {
  ...
  return <section className="section" id="quick"><SectionHead kicker="09 · quick access" title="旅行现场工具箱" note="把最容易临时查找的内容放在一起，点开就能用。" /><div className="tool-grid"><a className="card tool-card" href="https://www.google.com/maps" target="_blank" rel="noreferrer"><div className="tool-icon"><Map size={18} /></div><h3>地图导航</h3><p>Leaflet＋OpenStreetMap标记全程地点，Google Maps负责导航。</p></a><div className="card tool-card"><div className="tool-icon"><WalletCards size={18} /></div><h3>支付速查</h3><p>酒店押金Visa；商场Visa / 支付宝；夜市现金；榴莲现金优先。</p></div><a className="card tool-card" href="https://www.met.gov.my/" target="_blank" rel="noreferrer"><div className="tool-icon"><CloudSun size={18} /></div><h3>天气＆烟霾</h3><p>关注雷暴、风浪、Rough Seas、烟霾。查看METMalaysia / myCuaca。</p></a><div className="card tool-card"><div className="tool-icon">🍈</div><h3>榴莲攻略</h3><p>西马经典名品，沙巴隐藏角色；购买前确认RM/kg。</p></div></div><div className="card jetty-editor"><div><div className="micro">MY ACTUAL JETTY</div><h3>我的实际出海码头</h3><p>实际集合码头请以订单确认页为准。保存名称和Google Maps URL后，地图下方会出现快捷导航。</p></div><div className="jetty-form"><input value={jettyDraft.name} onChange={event => setJettyDraft(prev => ({ ...prev, name: event.target.value }))} placeholder="例如 South Jetty" /><input value={jettyDraft.url} onChange={event => setJettyDraft(prev => ({ ...prev, url: event.target.value }))} placeholder="Google Maps URL" /><button className="primary-btn small" onClick={() => setJetty(jettyDraft)}>保存集合点</button></div></div><div className="card" style={{ marginTop: 14, padding: 0, overflow: 'hidden' }}><MapView jetty={jetty} focusTarget={focusTarget} /></div><div className="card state-transfer"><div><div className="micro">TRAVEL STATE</div><h3>同���状态导入导出</h3><p>导出清单、排期、天气、收藏、预算、订票与MDAC状态，发给同行人后可在另一台设备导入。</p></div><div className="state-actions"><button className="primary-btn small" onClick={exportState}><FileJson size={14} />导出旅行状态</button><button className="ghost-btn small" onClick={() => fileRef.current?.click()}>导入JSON</button><input ref={fileRef} type="file" accept="application/json" hidden onChange={event => { const file = event.target.files?.[0]; if (file) importState(file) }} /></div></div></section>
}

function Safety() {
  return <section className="section" id="safety"><SectionHead kicker="13 · travel reminders" title="旅行提醒卡" note="不制造恐慌，只记住几个让旅程更舒服的小动作。" /><div className="card safety"><div className="safety-list">{['人多区域注意手机钱包', '手机不要长期靠马路一侧举着', 'Grab使用官方APP', '夜间避免偏僻小巷', '出海穿救生衣，海况不好不强行出海', '猴子区域不要拿食物逗猴', '海鲜购买前确认价格', '榴莲称重前确认RM/kg', '信用卡不要离开视线', '遇罚款要求official ticket / receipt'].map(item => <p key={item}>{item}</p>)}</div></div></section>
}
