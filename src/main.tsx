...
function Itinerary({ days, selected, setSelected, done, setDone, onToday, onOpenGallery, onMapFocus, overrides, onAdjust, onRemove, onRestoreSlot, onRestoreDay, onRestoreAll, onMove }: ItineraryProps) {
  ...
  return <section className="section" id="itinerary">
    ...
    <div className="date-tabs"><button className="today-btn" onClick={onToday}>📍 今天</button>{days.map((item, i) => { const isCustom = overrides.some(override => override.date === item.date) || item.items.some(entry => entry.source === 'custom'); return <button className={'date-tab ' + (selected === i ? 'active' : '') + (isCustom ? ' custom-tab' : '')} onClick={() => setSelected(i)} key={item.date}><strong>{item.date}</strong><span>{item.weekday}</span>{isCustom && <em>已自定义</em>}</button> })}</div>
